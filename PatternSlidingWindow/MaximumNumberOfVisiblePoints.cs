...
namespace PatternSlidingWindow
{
    public static class MaximumNumberOfVisiblePoints
    {
        ...
        public static void MainMaximumNumberOfVisiblePoints()
        {
            ...
            int[,] input = new int[,] { { 20, 10 }, { 26, 28 }, { 30, 55 } };
            int[,] location = new int[,] { { 1, 3 } };
            int angle = 90;
        ...
        public static int FindMaximumNumberOfVisiblePoints(int[,] input, int[,] location, int angle)
        {
            ...
            for (int i = 0; i < length0; i++)
            {
                ...
                sortedPolarAngles(normalizedInput[i, 0], normalizedInput[i, 1]);
            }
            Console.WriteLine($"\n----------------------");

            string polarAnglesToString = String.Join(", ", polarAngles);
            Console.WriteLine($"\n polarAngles = {polarAnglesToString}");
        ...
        public static void sortedPolarAngles(int x, int y)
        {

            bool isWritten = false;
            int n = 0;

            double polarAngle = Math.Atan2((double)y, (double)x) * 180 / Math.PI;

            Console.WriteLine($"\n--- CYCLE loop {polarAngle} is started, x = {x}, y = {y}, polarAngle = {(int)polarAngle}, isWritten = {isWritten}, n = {n}");
